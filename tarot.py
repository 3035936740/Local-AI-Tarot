...
def replace_string(msg: str):
    ...
    result = clean_redundant_punctuation(result)
    return result
...
def handle_element_text(element, a_mod: bool = False, total_zodiacs_key: set = set()):
    result_text = ""
...
# 选择一套牌阵
async def select_spreads(message: str):
    spreads_text = "可选的牌阵有:"
    ...
    out_spread_key = keep_english_digits(response['message']['content'].strip())
...
def getTarotInfo(spread_key, card_keys, elem_keys, court_elem_corr_keys, ast_mod_keys, zodiacs_keys):
    datas = {
        "spread": tarot_data['spreads'][spread_key],
        "cards": {},
        "astrologyModality": {},
    ...
    for card_key in card_keys:
        datas["cards"][card_key] = tarot_data["cards"][card_key]
        
    for elem_key in elem_keys:
        datas["elements"][elem_key] = tarot_data["elements"][elem_key]
        
    for court_elem_corr_key in court_elem_corr_keys:
        datas["courtElementalCorrespondence"][court_elem_corr_key] = tarot_data["courtElementalCorrespondence"][court_elem_corr_key]
        
    for ast_mod_key in ast_mod_keys:
        datas["astrologyModality"][ast_mod_key] = tarot_data["astrologyModality"][ast_mod_key]
        
    for zodiacs_key in zodiacs_keys:
        datas["zodiacs"][zodiacs_key] = tarot_data["zodiacs"][zodiacs_key]
...
async def tarot(user_message: str, a_mod: bool = False):
    global is_busy
    ...
    # 元素
    elements = tarot_data['elements']
    # 宫廷
    court_elemental_correspondence = tarot_data['courtElementalCorrespondence']

    #######################
    # 占星模式
    # 占星术情态
    astrology_modality = tarot_data['astrologyModality']
    # 十二星座
    zodiacs = tarot_data['zodiacs']
    if True:
    # try:
        if is_busy:
        ...
        user_message = remove_emojis(user_message)
        ...
        spread_key = await select_spreads(user_message)
        ...
        for index, card in enumerate(spread_positions):
            ...
            if first_element:
                ...
                tarot_texts += f"\n第一元素:{first_element_cn}," + handle_element_text(element, a_mod, total_zodiacs_key)
            if second_element:
                second_element_cn = tarot_card["second_element_cn"]
                total_elements.add(first_element)
                element = elements[first_element]
                tarot_texts += f"\n第二元素:{second_element_cn}," + handle_element_text(element, a_mod, total_zodiacs_key)
            if court_elemental:
                ...
                tarot_texts += f"\n宫廷元素:{court_element_cn},{court_name}含义:{court_meaning}," + handle_element_text(element, a_mod, total_zodiacs_key)
        ...
        for element_key in spread_elements:
            ...
            tarot_texts += "\n" + handle_element_text(element, a_mod, total_zodiacs_key)
        ...
        out_texts = replace_string(response["message"]["content"].strip()).split('。')
        ...
        datas = getTarotInfo(spread_key, random_cards, total_elements, total_court_elemental_correspondence_keys, total_astrology_modality_keys, total_zodiacs_key)
